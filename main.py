...
def main(strategy, map_, n_agents, attack_windows, time_horizons, l1_magnitude=0.1, batch_size=4, pos_weight=0.25, hidden_size=6):
    ...
    root = "/home/james/Desktop/000_thesis_magesim_results"

    extension = f"{strategy}/{map_}/{n_agents}_agent{'s' if n_agents > 1 else ''}"
    if not os.path.exists(f"{root}/adversarial_performance/tcml/{extension}"):
        os.makedirs(f"{root}/adversarial_performance/tcml/{extension}")

    th_results = [np.zeros(shape=len(attack_windows)) for _ in range(len(time_horizons))]

    for run in runs:

        idle_data = np.genfromtxt(f"{root}/adversarial_format_idleness/{extension}/{run}/idleness.csv", delimiter=',')[:, 1:]
        dist_data = np.genfromtxt(f"{root}/adversarial_format_idleness/{extension}/{run}/distance_metrics.csv", delimiter=',')[:, 1:]
        vel_data = np.genfromtxt(f"{root}/adversarial_format_idleness/{extension}/{run}/velocity_metrics.csv", delimiter=',')[:, 1:]
        vuln_data = np.genfromtxt(f"{root}/adversarial_format_idleness/{extension}/{run}/vulnerabilities.csv", delimiter=',')[:, 1:]
        ...
        for (th_ndx, time_horizon) in enumerate(time_horizons):
            ...
            for (aw_ndx, attack_window) in enumerate(attack_windows):
                ...
                np.savetxt(f"{root}/adversarial_performance/tcml/{extension}/{time_horizon}_{attack_window}_{run}.csv", outcomes, delimiter=',')

    for (th_ndx, time_horizon) in enumerate(time_horizons):
        np.savetxt(f"{root}/adversarial_performance/tcml/{extension}/{time_horizon}.csv", th_results[th_ndx], delimiter=',')
        np.save(f"{root}/adversarial_performance/tcml/{extension}/{time_horizon}.npy", th_results[th_ndx])
